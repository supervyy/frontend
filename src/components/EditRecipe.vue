...
const validations = computed(() => ({
  ...
  instructions: recipe.value.instructions.length > 0,
...
async function fetchRecipe(id: number) {
  try {
    ...
    recipe.value = {
      ...data,
      ingredients: data.ingredients.split(', ').filter((i: string) => i),
      instructions: data.instructions.split('. ').filter((i: string) => i),
    }
  } catch (err) {
...
const editingInstruction = ref<number | null>(null) // Add this line
const editedInstructionText = ref('') // Add this line
